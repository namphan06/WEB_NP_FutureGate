...
import { FiPlus, FiBriefcase, FiMapPin, FiClock, FiDollarSign, FiFileText, FiInfo, FiX, FiCheck, FiSearch } from 'react-icons/fi';
import {
    VIETNAM_PROVINCES,
    JOB_FIELDS,
    EMPLOYMENT_TYPES,
...
// Reusing the same styles as employer form if possible, or we can copy/adjust
import '../employer/JobFormStyles.css';

export default function SchoolCreateJobPage() {
    const { user } = useAuth();
        ...
        try {
            ...
            const { error } = await supabase
                ...
                .insert([{
                    ...
                    status: 'approved' // Automatically approve school posts for now, or match employer's 'pending'
                }]);
        ...
        <div className="job-form-page">
            <div className="container" style={{ paddingTop: '2rem', paddingBottom: '1rem' }}>
                <h1 style={{ fontSize: '1.5rem', fontWeight: '700', color: '#212f3f', marginBottom: '0.5rem' }}>
                    Đăng tin tuyển dụng (Nhà trường)
                </h1>
                <p style={{ color: '#6c757d', fontSize: '0.9375rem', marginBottom: 0 }}>
                    Hỗ trợ doanh nghiệp đối tác hoặc tin tuyển dụng riêng của nhà trường
                </p>
            </div>
                        ...
                        <div className="form-group">
                            ...
                            <div className="input-with-button">
                                <input
                                    ...
                                    placeholder="VD: Phân điểm A, KTX..."
                            ...
                            <div className="item-list">
                                {formData.work_locations.map((loc, idx) => (
                                    <div key={idx} className="item-card">
                                        ...
                                        <button type="button" onClick={() => removeItem(idx, 'work_locations')} className="item-remove">
                                            <FiX size={16} />
                                        </button>
                                    </div>
                                ))}
                        ...
                        <div className="form-group">
                            ...
                            <div className="input-with-button">
                                <textarea
                                    ...
                                    rows={3}
                                />
                                <button type="button" className="btn btn-icon" onClick={() => addItem(newJobDesc, setNewJobDesc, 'job_description')}>
                                    <FiPlus />
                                </button>
                            </div>
                            <div className="item-list">
                                {formData.job_description.map((desc, idx) => (
                                    <div key={idx} className="item-card">
                                        ...
                                        <button type="button" onClick={() => removeItem(idx, 'job_description')} className="item-remove">
                                            <FiX size={16} />
                                        </button>
                                    </div>
                                ))}
                        ...
                        <div className="form-group">
                            ...
                            <div className="input-with-button">
                                <textarea
                                    ...
                                    placeholder="Yêu cầu kỹ năng, thái độ..."
                                    rows={3}
                                />
                                <button type="button" className="btn btn-icon" onClick={() => addItem(newRequirement, setNewRequirement, 'candidate_requirements')}>
                                    <FiPlus />
                                </button>
                            </div>
                            <div className="item-list">
                                {formData.candidate_requirements.map((req, idx) => (
                                    <div key={idx} className="item-card">
                                        ...
                                        <button type="button" onClick={() => removeItem(idx, 'candidate_requirements')} className="item-remove">
                                            <FiX size={16} />
                                        </button>
                                    </div>
                                ))}
